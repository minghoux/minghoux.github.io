# Goal: Abound 2026 loan application rebrand
Review the existing Abound loan application journey and apply the new branding to the expreience.

## Knowledge Sources
- **Master Design System**: `abound-brand-2026/design-system/abound-design-system.md`
- **Brand Identity**: Abound 2026 guidelines (Saans Typography, Forest/Lime/Teal palette).
- **Reference Logic**: `ibancredit/current-journey` (Used for initial flow parity).
...
## Design Standard: Refined Fintech (Flat UI)
The journey targets a "Refined Fintech" aesthetic — premium and trustworthy without being cold. Key principles:

- **Shadowless UI (Core)**: Completely removed drop shadows on components. Hierarchy is driven by 1.5px borders (`neutral-200` for idle, `brand` for active) and semantic spacing.
...
- **Rounded CTAs (14px)**: Primary buttons use a professional rounded-rectangle shape (`border-radius: 14px`). This aligns with the input field corner radius for a cohesive, "built" feel. Avoid pill shapes to maintain the refined fintech aesthetic.
- **Brand Lime as Accent**: `#D4F19B` (lime) is the primary functional accent — used in the progress bar fill, the calculator result box, and the outcome screen. Not to be confused with the semantic `success` green.
- **Immersive Header**: Persistent Forest Green (`#0E4533`) header with centered Abound wordmark and a lime progress bar (no glow).
...
8. [x] **Phase 8: Visual Refinement** — Differentiated selection patterns, Rounded CTAs (14px), lime accent system.
9. [ ] **Phase 9: Identity Verification (IDV)** — Integration of specialized IDV screens for DNI/NIE scanning.
10. [ ] **Phase 10: Bank Logomark Integration** — Replacing generic icons with verified bank marks for the Bancos step.
